...
class PollReader():
    ...
    def build_data_dict(self):
        ...
        for i in self.raw_data[1:]:
            ...
            seperated = i.rstrip().split(',') 
            sample_num_str, sample_type = seperated[2].split()
            ...
            self.data_dict['sample'].append(int(sample_num_str))
    ...
    def polling_history_change(self):
        ...
        latest_h = harris[:30]
        latest_t = trump[:30]
        earliest_h = harris[-30:]
        earliest_t = trump[-30:]

        changeforharris = (sum(latest_h) / len(latest_h)) - (sum(earliest_h) / len(earliest_h))
        changefortrump = (sum(latest_t) / len(latest_t)) - (sum(earliest_t) / len(earliest_t))
        return changeforharris, changefortrump
